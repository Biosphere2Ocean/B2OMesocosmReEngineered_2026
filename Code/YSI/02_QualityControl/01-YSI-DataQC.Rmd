...
df <- read_csv("Data/Clean/YSI/01_DataWrangling/02-YSI-Data-Clean-All.csv") %>%
  mutate(`Chlorophyll (RFU)` = as.double(`Chlorophyll (RFU)`),
...
dfQCCutoff <- dfQC %>%
  ...
  mutate(`Salinity (PSU)` = ifelse(Date > "2020/01/01" & `Salinity (PSU)` < 31, NA, `Salinity (PSU)`)) %>%
  mutate(`Salinity (PSU)` = ifelse(Date < "2013/01/01" & `Salinity (PSU)` < 24, NA, `Salinity (PSU)`)) %>%
  mutate(`Temperature (ºC)` = ifelse(`Temperature (ºC)` < 20 | `Temperature (ºC)` > 28, NA, `Temperature (ºC)`))
