...
dfQCCutoff <- dfQC %>%
  ...
  mutate(pH = ifelse(pH < 7.0, NA, pH)) %>%
  mutate(`Salinity (PSU)` = ifelse(Date >= "2020/01/01" & Date <= "2021-01-01" & `Salinity (PSU)` < 31, NA, `Salinity (PSU)`)) %>%
  mutate(`Salinity (PSU)` = ifelse(Date <= "2013/01/01" & Date >= "2012-01-01" & `Salinity (PSU)` < 25, NA, `Salinity (PSU)`)) %>%
  mutate(`Temperature (ºC)` = ifelse(Date <= "2014-10-31" & `Temperature (ºC)` < 20, NA, `Temperature (ºC)`))
...
for (i in 1:length(var_names)) {
  plotList_postqccutoff[[i]] <- dfQCPivot %>%
    filter(variable == var_names[i]) %>%
    ggplot(aes(x = Date,
               y = variable_value)) +
    geom_point() +
